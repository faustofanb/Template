...
import fausto.fan.project.framework.starter.idempotent.enums.IdempotentMQConsumeStatusEnum
import fausto.fan.project.framework.starter.idempotent.toolkit.LogUtil
import fausto.fan.project.framework.starter.idempotent.toolkit.SpELUtil
...
class IdempotentSpELByRestAPIExecuteHandler(
    ...
    override fun handler(wrapper: IdempotentParamWrapper) {
        ...
        if(!lock.tryLock())
            throw ClientException(wrapper.idempotent!!.message)
...
class IdempotentSpELByMQExecuteHandler(
     ...
     */
    companion object {
        private const val TIMEOUT: Int = 600
