 ...
 * 该对象池主要用于存储和管理单例对象，通过键值对的形式进行存储，键为对象的标识，值为对象本身。
 */
object SingletonObjectPool {
    ...
    fun <T> get(key: String, supplier: (() -> T)? = null): T? {
        var result = SINGLE_OBJECT_POOL[key]
        if (result == null && supplier != null) {
            result = supplier.invoke()?.let {
                SINGLE_OBJECT_POOL[key] = it
            }
        }
        return result as? T
    }
    ...
    fun put(key: String? = null, value: Any) {
        ...
        val actualKey = key ?: value.javaClass.name
        // 将值放入单例对象池中，使用实际的键进行索引
        SINGLE_OBJECT_POOL[actualKey] = value
    }
}
